...
const SHA256Hash: FC<SHA256Hash> = ({ data, toHash }) => {
  ...
  return (
    <div className="flex flex-col md:flex-row items-center gap-2 -mt-4">
      ...
      <span className="w-fit text-sm bg-gray-100 p-2 rounded-xl text-clip xl:hidden">
        {truncated(dataToShow)}
      </span>
      <span className="w-fit bg-gray-100 text-xs p-2 rounded-xl text-clip hidden xl:flex">
        {dataToShow}
      </span>
    </div>
  );
